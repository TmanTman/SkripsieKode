...
def GetLoadProfile(ProfileID,LoadStat,ScheduleStat):
    ...
    Query = '''SELECT profiletimestamps.Timestamp FROM profiletimestamps INNER JOIN profiledata ON profiletimestamps.ID = profiledata.TimestampID WHERE profiledata.ProfileID = %s;''' % (ProfileID)
    cur.execute(Query)
...
LoadStat = True
CycleStat = True
Timestamp,Profile = GetLoadProfile(ProfileID,LoadStat,CycleStat)
print "Timestamp [hh:mm:ss]=",Timestamp,"\n"      
print "Timestamp [sec]=",Profile[0,:],"\n"
print "Energy=",Profile[1,:],"\n"
print "Demand=",Profile[2,:],"\n"

XTickInt = 4
barplot(Timestamp,Profile[1,:],XTickInt)
